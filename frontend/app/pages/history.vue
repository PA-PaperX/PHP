...
const { data, pending } = await useApi<any>('/api/auth/history.php')

const historyItems = computed(() => data.value?.history || [])
    ...
    <UCard>
      ...
      <div v-else class="space-y-4">
        <div v-for="item in historyItems" :key="`${item.type}-${item.id}`" 
             class="flex items-start gap-4 p-4 rounded-xl border border-gray-100 dark:border-gray-800 bg-white dark:bg-gray-950 hover:shadow-md transition-shadow">
          <div :class="[
