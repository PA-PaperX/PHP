...
// Data fetching
const { data: equipResponse, pending: equipPending, refresh: refreshEquip } = await useApi<any>('/api/inventory/index.php')
const { data: borrowsResponse, pending: borrowsPending, refresh: refreshBorrows } = await useApi<any>('/api/borrows/index.php')

const equipments = computed(() => equipResponse.value?.equipment || [])
const borrows = computed(() => borrowsResponse.value?.borrows || [])
...
const saveEquipment = async () => {
  ...
  try {
    ...
    const endpoint = form.value.id ? '/api/inventory/update.php' : '/api/inventory/create.php';
...
const confirmDelete = async () => {
  ...
  try {
    const baseUrl = useBaseUrl();
    await $fetch(`${baseUrl}/api/inventory/delete.php`, {
      method: 'POST',
...
const fetchSilentUpdates = async () => {
  ...
  try {
    ...
    if (equipData?.equipment) equipResponse.value = equipData;
    if (borrowsData?.borrows) borrowsResponse.value = borrowsData;
  } catch (e) {
    // Silent fail
  }
}
    ...
    <UTabs :items="items" class="w-full font-kanit">
      ...
      <template #equipment>
        <UCard class="mt-4" :ui="{ body: { padding: '!p-0' } }">
          ...
          <div id="admin-inventory-table" class="overflow-x-auto w-full">
      ...
      <template #borrows>
        <UCard class="mt-4" :ui="{ body: { padding: '!p-0' } }">
          ...
          <UTable :data="borrows" :columns="borrowColumns" :loading="borrowsPending">
            ...
            <template #actions-cell="{ row }">
              <UButton size="xs" color="gray" variant="ghost" icon="i-heroicons-eye" @click="openBorrowDetail(row.original)">
                ดู / จัดการ
              </UButton>
            </template>
          </UTable>
      ...
      <template #content>
        <div class="flex flex-col h-[100dvh] overflow-hidden bg-white dark:bg-gray-950 font-kanit">
          ...
          <div class="flex-1 overflow-y-auto p-6 space-y-6" v-if="selectedBorrow">
            ...
            <div class="grid grid-cols-2 gap-4 mt-4">
              <div>
                <p class="text-sm text-gray-500">วันที่ยืม</p>
                <p class="font-medium">{{ selectedBorrow.borrow_date }}</p>
              </div>
              <div>
                <p class="text-sm text-gray-500">กำหนดคืน</p>
                <p class="font-medium">{{ selectedBorrow.return_date || '-' }}</p>
              </div>
            </div>
