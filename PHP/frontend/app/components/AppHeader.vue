...
const fetchNotifications = async () => {
  ...
  try {
    const res = await $fetch<any>('/api/notifications/index', {
      baseURL: useBaseUrl(),
      credentials: 'include'
    })
...
<template>
  ...
  >
    <nav :class="['flex items-center justify-between px-4 sm:px-6 transition-all duration-300 w-full mx-auto max-w-7xl', scrolled && !isOpen ? 'h-14' : 'h-16']">
      ...
      <div class="flex items-center gap-6 shrink-0 flex-1">
        <!-- Logo -->
      ...
      <div class="flex items-center justify-end gap-2 sm:gap-4 shrink-0">
        ...
        <div class="hidden md:flex items-center">
          <UButton 
            variant="ghost" 
            color="gray"
            class="text-gray-400 hover:text-gray-600 dark:hover:text-gray-300 flex items-center gap-2 px-3 py-1.5 rounded-full bg-gray-50 dark:bg-gray-800/50 hover:bg-gray-100 dark:hover:bg-gray-800 border border-gray-200 dark:border-gray-700 font-kanit transition-all"
          >
            <UIcon name="i-heroicons-magnifying-glass" class="w-4 h-4" />
            <span class="text-xs">ค้นหา...</span>
            <UKbd class="ml-2 hidden lg:inline-block">Ctrl K</UKbd>
          </UButton>
        </div>
