...
const { data: recentIssues, pending } = await useApi<any>('/api/issues/index', { query: { limit: 5 } })
...
const fetchSilentUpdates = async () => {
  ...
  try {
    ...
    const data = await $fetch<any>('/api/issues/index?limit=5', { baseURL: baseUrl, credentials: 'include' });
  ...
  <div class="space-y-6 pb-10" v-motion-fade>
    ...
    <div v-if="user?.role !== 'admin'" class="grid grid-cols-1 sm:grid-cols-2 gap-6" v-motion-slide-visible-bottom>
      <NuxtLink to="/report-issue" class="block h-full">
        <UCard class="h-full dashboard-surface dashboard-surface-hover cursor-pointer group">
          <div class="flex flex-col items-center text-center gap-4 p-6">
            <div class="dashboard-icon-pill bg-primary-600 dark:bg-primary-500 p-4 text-white shadow-md shadow-primary-500/30 group-hover:scale-110 transition-transform">
      ...
      <NuxtLink to="/inventory" class="block h-full">
        <UCard class="h-full dashboard-surface dashboard-surface-hover cursor-pointer group">
          <div class="flex flex-col items-center text-center gap-4 p-6">
            <div class="dashboard-icon-pill bg-slate-800 dark:bg-slate-700 p-4 text-white shadow-md shadow-slate-900/20 group-hover:scale-110 transition-transform">
    ...
    <div class="mt-8" v-motion-fade :delay="400">
      ...
      <UCard class="dashboard-surface overflow-hidden" :ui="{ body: { padding: '!p-0' } }">
        <div v-if="pending" class="p-6 space-y-4">
          ...
          <USkeleton class="h-10 w-full rounded-lg opacity-70" />
          <USkeleton class="h-10 w-full rounded-lg opacity-40" />
        </div>
        <UTable 
          v-else 
          :data="activeIssues" 
          :columns="columns" 
          class="font-kanit" 
          :empty-state="{ icon: 'i-heroicons-document-text', label: '📄 ยังไม่มีข้อมูลประวัติการแจ้งปัญหา' }"
    ...
    <USlideover v-model:open="isSlideoverOpen" :ui="{ content: 'w-screen max-w-md', overlay: { background: 'bg-gray-900/50 backdrop-blur-sm' } }">
      <template #content>
        <div class="flex flex-col h-full bg-white dark:bg-gray-950 font-kanit">
          ...
          <div class="p-4 border-t border-gray-100 dark:border-gray-800 flex gap-3 bg-gray-50 dark:bg-gray-900">
            <UButton color="gray" variant="solid" class="w-full justify-center" @click="isSlideoverOpen = false">ปิดหน้าต่าง</UButton>
            <UButton color="primary" variant="solid" class="w-full justify-center" :to="`/issue/${selectedIssue?.id}`">ดูแบบเต็ม / ชำระเงิน</UButton>
          </div>
        </div>
      </template>
    </USlideover>
