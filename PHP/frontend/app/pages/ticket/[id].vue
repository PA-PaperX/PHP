...
const fetchMessages = async () => {
  try {
    const data = await $fetch<any>(`${baseUrl}/api/tickets/messages?ticket_id=${ticketId}&token=${token}&last_id=${lastMessageId}`)
...
const sendMessage = async () => {
  ...
  try {
    await $fetch(`${baseUrl}/api/tickets/messages?ticket_id=${ticketId}&token=${token}`, {
      method: 'POST',
      body: { message: text }
    })
