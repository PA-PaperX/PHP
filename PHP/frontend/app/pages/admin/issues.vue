...
const { data: issuesResponse, pending, refresh } = await useApi<any>('/api/issues/index')

const issues = computed(() => {
  const res = issuesResponse.value
  if (!res) return []
...
const updateIssue = async (closeOnSuccess = true) => {
  ...
  try {
    ...
    await $fetch(`${baseUrl}/api/issues/update`, {
      method: 'POST',
      body: formData,
      credentials: 'include'
    })
...
const fetchSilentUpdates = async () => {
  ...
  try {
    const baseUrl = useBaseUrl();
    const data = await $fetch<any>('/api/issues/index', { baseURL: baseUrl, credentials: 'include' });
    ...
    <UCard :ui="{ body: { padding: '!p-0' } }" v-motion-fade :delay="100">
      <div id="admin-issues-table">
        ...
        <div class="hidden md:block overflow-x-auto w-full">
          <UTable :data="filteredIssues" :columns="columns" class="font-kanit min-w-[800px]" :loading="pending">
        ...
        <div class="md:hidden flex flex-col divide-y divide-gray-100 dark:divide-gray-800 w-full font-kanit">
          ...
          <div v-else-if="filteredIssues.length === 0" class="p-8 text-center text-gray-500 flex flex-col items-center">
             <UIcon name="i-heroicons-inbox" class="w-12 h-12 mb-2 text-gray-200 dark:text-gray-700" />
             ไม่มีข้อมูลปัญหา
          </div>
          <div v-else v-for="issue in filteredIssues" :key="issue.id" class="p-4 flex flex-col gap-3">
            <div class="flex justify-between items-start gap-2">
              <div class="min-w-0 flex-1">
                <div class="font-bold text-gray-900 dark:text-white truncate text-base">#{{ issue.id }} {{ issue.title }}</div>
                <div class="text-sm text-gray-500 mt-0.5 truncate">{{ issue.user_email }}</div>
              </div>
              <div class="flex flex-col gap-1 items-end shrink-0">
                <UBadge :color="issue.status === 'resolved' ? 'success' : (issue.status === 'closed' || issue.status === 'cancelled') ? 'red' : issue.status === 'in_progress' ? 'primary' : 'warning'" variant="subtle" size="xs">
                  {{ issue.status === 'resolved' ? 'เสร็จสิ้น' : (issue.status === 'closed' || issue.status === 'cancelled') ? 'ยกเลิก' : issue.status === 'in_progress' ? 'กำลังดำเนินการ' : 'รอดำเนินการ' }}
                </UBadge>
                <UBadge :color="issue.payment_status === 'paid' ? 'success' : 'warning'" variant="subtle" size="xs">
                  {{ issue.payment_status === 'paid' ? 'ชำระแล้ว' : 'รอชำระ' }}
                </UBadge>
              </div>
            </div>
            
            <div class="flex items-center gap-2 text-sm text-gray-500 bg-gray-50 dark:bg-gray-800/50 p-2 rounded-lg border border-gray-100 dark:border-gray-800">
              <UIcon name="i-heroicons-map-pin" class="w-4 h-4 shrink-0" />
              <span class="truncate">{{ issue.location }}</span>
            </div>

            <UButton block size="sm" color="gray" variant="soft" icon="i-heroicons-eye" @click="openIssue(issue)">
              ดูรายละเอียด / จัดการ
            </UButton>
          </div>
        </div>
